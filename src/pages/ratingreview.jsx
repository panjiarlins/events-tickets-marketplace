...
const validationSchema = Yup.object({
  rating: Yup.number().min(1, 'Select a rating').max(5, 'Select a rating').required('Rating is required'),
  review: Yup.string().trim().max(100,'max 100 character').required('Review is required'),
});
        ...
        >
          {({ values, setFieldValue }) => (
            <Form>
              ...
              <div>
                <Field as="textarea" name="review" rows="4" cols="50" />
                <ErrorMessage name="review" component="div" className="error" />
              </div>
              <Button colorScheme='blackAlpha'type='submit'>Button</Button>
            </Form>
          )}
        </Formik>
      </div>
              ...
              <div>
                ...
                <div style={{ display: 'flex' }}>
                  {[1, 2, 3, 4, 5].map((ratingValue) => (
                    <FaStar
                      key={ratingValue}
                      color={ratingValue <= reviewItem.rating ? 'gold' : 'gray'}
                    />
                  ))}
                </div>
              </div>
